...
(function ($) {
  $(document).ready(function () {
    ...
    $("#datepicker").datepicker({
      ...
      changeMonth: !mobile ? true : false,
      dateFormat: 'dd/mm/yy',
      changeYear: !mobile ? true : false,
      monthNamesShort: $('html[lang="he"]').length ? $.datepicker.regional["he"].monthNames : $.datepicker.regional["en"].monthNames,
      beforeShowDay: $.datepicker.noWeekends,
      onSelect: function (dateText, inst) {
        $(this).data('datepicker').inline = true;
        setTimeout(insertHeader, 5);
        setTimeout(insertFooter, 6);
        setTimeout(CalendarFooterIndicator, 7);
        setTimeout(condenceFooter, 8);
      },
      onClose: function () {
        $(this).data('datepicker').inline = false;
        let date_value = $('input#datepicker').val();
        if (date_value.length === 0) {
          $('.days-dropdown-select .base-line').removeClass('marked');
          $('.' + lastSelected).addClass('marked');
        }
      },
      beforeShow: function (textbox, instance) {
        $('.days-dropdown-select').append($('#ui-datepicker-div'));
        setTimeout(insertHeader, 5);
        setTimeout(insertFooter, 6);
        setTimeout(CalendarFooterIndicator, 7);
        setTimeout(condenceFooter, 8);
        setTimeout(function () {
          if (mobile) $('#ui-datepicker-div').addClass('mobile-mode');
        }, 8);
      },
      onChangeMonthYear: function (year, month, inst) {
        ...
        setTimeout(function () {
          condenceFooter();
          let month_text = $('.ui-datepicker-month option[selected="selected"]').text();
          let month_val = $('.ui-datepicker-month option[selected="selected"]').val();
          let year_text = $('.ui-datepicker-year option[selected="selected"]').text();
          let year_val = $('.ui-datepicker-year option[selected="selected"]').val();
          $('.datepicker-head .dropdown.months button').text(month_text);
          $('.datepicker-head .dropdown.years button').text(year_text);
          $('.datepicker-head .dropdown ul li').removeClass('selected');
          $('.datepicker-head .dropdown.months ul li[data-value="' + month_val + '"').addClass('selected');
          $('.datepicker-head .dropdown.years ul li[data-value="' + year_val + '"').addClass('selected');
        }, 8);
      }
    });

    $(document).on("mouseover", "a.ui-corner-all", function () {
      if ($(this).hasClass("ui-datepicker-next-hover")) {
        $(this).removeClass('ui-datepicker-next-hover');
        $(this).removeClass('ui-state-hover');
      }
      if ($(this).hasClass("ui-datepicker-prev-hover")) {
        $(this).removeClass('ui-datepicker-prev-hover');
        $(this).removeClass('ui-state-hover');
      }
    });

    var minDayNames = mobile ? [Drupal.t('ראשון'), Drupal.t('שני'), Drupal.t('שלישי'), Drupal.t('רביעי'), Drupal.t('חמישי'), Drupal.t('שישי'), Drupal.t('שבת')] : [Drupal.t('א'), Drupal.t('ב'), Drupal.t('ג'), Drupal.t('ד'), Drupal.t('ה'), Drupal.t('ו'), Drupal.t('ש')];
    $.datepicker.regional['he'] = {
      dayNamesMin: minDayNames,
    };
    let datePickerRegional = $('html[lang="he"]').length ? $.datepicker.regional["he"] : $.datepicker.regional["en"];
    $.datepicker.setDefaults(datePickerRegional);


    $("#datepicker").mousedown(function () {
      lastSelected = $('.days-filters .marked').data('filter-type');

      let baseLineEl = $('.days-dropdown-select .base-line');

      $('#ui-datepicker-div').toggle();
      $('.filters-container button.filter-item').removeClass('marked');

      let date_value = $('input#datepicker').val();

      if (baseLineEl.hasClass('marked') && date_value.length === 0) {
        baseLineEl.removeClass('marked');
      } else {
        baseLineEl.addClass('marked');
      }
    });

    $(document).on('click', '#reset-date', function () {
      resetDatePicker();
      setTimeout(insertFooter, 10);
      setTimeout(CalendarFooterIndicator, 10);
      triggerTodayDate();
      if (!mobile) {
    ...
    $(document).on('click', '#choose-date', function () {
      ...
      if (date_value === '') {
        $('.filter-item.today').click();
      } else {
        let calendar_unit_arr = date_value.split('/');
        var date_to_filter = calendar_unit_arr[2] + '-' + calendar_unit_arr[1] + '-' + calendar_unit_arr[0];
        if ($('#fer-table-content table tbody tr[data-date="' + date_to_filter + '"]').length > 0) {
          chooseDateMark(date_value, filter_type);
        } else {
          $.ajax({
            url: "/" + drupalSettings.path.pathPrefix + "coin-rates?date=" + date_to_filter,
            cache: false,
            success: function (result) {
              if (result && result.length) {
                createCoinTable(result);
                setTimeout(function () {
                  chooseDateMark(date_value, filter_type);
                }, 10);
              }
    ...
    function chooseDateMark(date_value, filter_type) {
      $('.days-filters button').removeClass('marked');
      filterDate(filter_type, false);
      ValidityAndPublish();
      $('.days-dropdown-select .base-line span').text(date_value);
    }

    $('body').on('click', 'button.filter-item', function () {
      resetDatePicker();
      filterDate(filter_type, $(this));
      ValidityAndPublish();
    });

    $(document).on('click', '.show-more-coins span', function () {
      $(this).parent().toggleClass('closed');
      $('.far-mobile-content').toggleClass('popular-items');
    });
    //end of datepicker


    //tooltip
    $('#precent-tooltip').tooltip({
      position: {
        my: "center bottom",
        at: "right-7 top-3"
      },
      tooltipClass: "poalim-tooltip",
    });


    //ajax call
    $.ajax({
      url: "/" + drupalSettings.path.pathPrefix + "coin-rates",
      cache: true,
      success: function (result) {
        if (result && result.length) {
          var firstItem = result[0];
          createCoinTable(result.sort(sortByTax));
          setTimeout(function () {
            setDefaultDateReceived(firstItem)
          }, 10);
        }
      }
    });

    function sortByTax(a, b) {
      if (a.TAX_ORDER < b.TAX_ORDER) {
        return -1;
      }
      if (a.TAX_ORDER > b.TAX_ORDER) {
        return 1;
    ...
    function setDefaultDateReceived(result) {

      var lastDateFromResult = result["DT_ERECH"];
      $('#datepicker').datepicker("setDate", new Date(lastDateFromResult));
    ...
    //bootstrap month and year select boxes behaviour
    $('body').on('click', '.datepicker-head .dropdown ul li', function () {
      let ul_text = $(this).closest('.dropdown').find('button').text();
      let li_text = $(this).text();

      //trigger original select
      let dropdown_type = $(this).closest('.dropdown').hasClass('months') ? 'months' : 'years';
      let data_value = $(this).data('value');
      //set selected propr
      let $hiddenDropdownMonths = $('.hidden-datepicker .dropdown.months ul li');
      let $hiddenDropdownYears = $('.hidden-datepicker .dropdown.years ul li');
      if (dropdown_type === 'months') {
        $hiddenDropdownMonths.removeClass('selected');
        $hiddenDropdownMonths.eq($(this).index()).addClass('selected');
      }
      if (dropdown_type === 'years') {
        $hiddenDropdownYears.removeClass('selected');
        $hiddenDropdownYears.eq($(this).index()).addClass('selected');
      }
      ...
      let text = $(this).text();
      var date = $('#datepicker').datepicker('getDate') ? $('#datepicker').datepicker('getDate') : new Date();

      if (dropdown_type === 'months') {
        $('.hidden-datepicker .dropdown.months button').text(text);
        date.setMonth(data_value);
        $("#datepicker").datepicker("setDate", date);
      }
      if (dropdown_type === 'years') {
        $('.hidden-datepicker .dropdown.years button').text(text);
        date.setFullYear(data_value);
        $("#datepicker").datepicker("setDate", date);
      }
  ...
  function createCoinTable(result) {

    var matachRates = JSON.parse($(".far-wrapper")[0].getAttribute('data-rates'));
    var idx = 0;
    var popular = '';
    var popular_coins = [];
    result.forEach(function (element) {

      let KOD_MATBEA = element['KOD_MATBEA'];
      let matachItem = matachRates.find(function(item){
        return item['matach_id'] == KOD_MATBEA
      } );
      let COIN_IMG = matachItem['coin_image'][0];
      let COIN_IMG_URL = matachItem['coin_image_url'];
      let NAME_HEB = element['NAME_HEB'];
      let DT_ERECH = element['DT_ERECH'];
      let DT_ERECH_ORDERED = dateOrdered(DT_ERECH);
      let DT_VALID = element['DT_VALID'];
      let DT_VALID_ORDERED = dateOrdered(DT_VALID);
      let SHAAR_CASH_KNIA = element['SHAAR_CASH_KNIA'];
      let SHAAR_CASH_MECHIRA = element['SHAAR_CASH_MECHIRA'];
      let SHAAR_CHK_KNIA = element['SHAAR_CHK_KNIA'];
      let SHAAR_CHK_MECHIRA = element['SHAAR_CHK_MECHIRA'];
      let SHAAR_YATZIG = element['SHAAR_YATZIG'];
      let AHUZ_SHINUI = element['AHUZ_SHINUI'];
      let precent_class = (AHUZ_SHINUI > 0) ? "positive" : (AHUZ_SHINUI < 0) ? "negative" : "even";

      if (popular_coins.indexOf(KOD_MATBEA) === -1) {
        idx++;
        if (idx <= 3) popular_coins.push(KOD_MATBEA);
      }

      popular = popular_coins.indexOf(KOD_MATBEA) > -1 ? 'popular' : '';

      let row = '<tr class="hide" data-date="' + DT_ERECH + '" data-validity="' + DT_VALID + '">' +
        '<td class="name-and-img">';

      if(COIN_IMG && COIN_IMG_URL) {
        row += '<div class="img-wrapper"><img src="'+COIN_IMG_URL+'" alt="'+ COIN_IMG['alt'] +'" title="'+ COIN_IMG['title'] +'" ></div>';
      }
        row += '<span>'+NAME_HEB+'</span>'  + '</td>' +
        '<td>' + SHAAR_YATZIG + '</td>' +
        '<td class="change ' + precent_class + '"><span>' + AHUZ_SHINUI + '%</span></td>' +
        '<td>' + SHAAR_CHK_KNIA + '</td>' +
        '<td>' + SHAAR_CHK_MECHIRA + '</td>' +
        '<td>' + SHAAR_CASH_KNIA + '</td>' +
        '<td>' + SHAAR_CASH_MECHIRA + '</td>' +
        '</tr>';

      $('#fer-table-content table tbody').append(row);

      //MOBILE VERSION
      if ($('.far-mobile-content').is(':visible')) {
        var rubric = "<div class=\"coin-rate-wrapper hide " + popular + "\" data-date=\"" + DT_ERECH + "\" data-validity=\"" + DT_VALID + "\">\n<div class=\"cr-title\">";
        if(COIN_IMG && COIN_IMG_URL) {
          rubric += '<img src="'+COIN_IMG_URL+'" alt="'+ COIN_IMG['alt'] +'" title="'+ COIN_IMG['title'] +'" >';
        }

        rubric+='<span>'+NAME_HEB+'</span>'  + "</div>\n<div class=\"cr-yatzig-wrap\">\n  <div class=\"cr-publish-date\">" + Drupal.t("Publication date ") + DT_VALID_ORDERED + "</div>\n  <div class=\"cr-yatzig\">" + Drupal.t("Representative rate: ") + SHAAR_YATZIG + "</div>\n</div>\n<div class=\"cr-trade-day-wrap\">\n  <div class=\"day-trade\">" + Drupal.t("Trading day: ") + DT_ERECH_ORDERED + "</div>\n  <div class=\"precent-change\">\n    <span class=\"precent-title\">" + Drupal.t("Change:") + "</span>\n    <span class=\"precent-value " + precent_class + "\">" + AHUZ_SHINUI + "%</span>\n  </div>\n  <div class=\"chk-cash-values\">\n    <div class=\"chk-cash-value-wrap\">\n<div class=\"chk-cash-title\">" + Drupal.t("Checks – buy") + "</div>\n<div class=\"chk-cash-value buy-chk\">" + SHAAR_CHK_KNIA + "</div>\n    </div>\n    <div class=\"chk-cash-value-wrap\">\n<div class=\"chk-cash-title\">" + Drupal.t("Checks – sell") + "</div>\n<div class=\"chk-cash-value sell-chk\">" + SHAAR_CHK_MECHIRA + "</div>\n</div>\n<div class=\"chk-cash-value-wrap\">\n<div class=\"chk-cash-title\">" + Drupal.t("Cash – buy") + "</div>\n<div class=\"chk-cash-value buy-cash\">" + SHAAR_CASH_KNIA + "</div>\n    </div>\n    <div class=\"chk-cash-value-wrap\">\n<div class=\"chk-cash-title\">" + Drupal.t("Cash – sell") + "</div>\n<div class=\"chk-cash-value sell-cash\">" + SHAAR_CASH_MECHIRA + "</div>\n    </div>\n  </div>\n</div>\n      </div>";
      }

      $('.far-mobile-content').append(rubric);

    });
  }

  //values for validity and publish date fields
  function ValidityAndPublish() {
    let $visible_rows = $('#fer-table-content table tbody tr:not(.hide)');
    let data_from_tr = $visible_rows.is(':visible');

    if (data_from_tr) {
      let publishDate = $visible_rows.data('date');
      let validityDate = $visible_rows.data('validity');
      let unit_arr_publish = publishDate.split('-');
      let unit_arr_validity = validityDate.split('-');
      let publishDateOrdered = unit_arr_publish[2] + '/' + unit_arr_publish[1] + '/' + unit_arr_publish[0];
      let validityDateOrdered = unit_arr_validity[2] + '/' + unit_arr_validity[1] + '/' + unit_arr_validity[0];

      $('#fer-table-content thead .publish-date span').text(publishDateOrdered);
      $('#fer-table-content thead .trade-date span').text(validityDateOrdered);
    }
  }
  ...
  //insert footer to datepicker
  function insertFooter() {
    const is_mobile = window.innerWidth <= 950;
    var btn_text1, btn_text2;
    if ($('html[lang="he"]').length) {
      btn_text1 = 'בחר';
      is_mobile ? btn_text2 = 'אפס' : btn_text2 = 'לתאריך של היום';
    } else {
      btn_text1 = 'Choose';
      is_mobile ? btn_text2 = 'Reset' : "To today's date";
    }
    const footer_html = '<div class="datepicker-footer"><div class="poalim-red-btn" id="choose-date">' + btn_text1 + '</div></div>';
    if ($('.datepicker-footer').length === 0) {
      $('#ui-datepicker-div').append(footer_html);
    }
  }

  //insert header to datepicker
  function insertHeader() {
    var html = $('.datepicker-head').parent().html();
    if (!$('#ui-datepicker-div .ui-datepicker-title .datepicker-head').length) {
      $('#ui-datepicker-div .ui-datepicker-title').prepend(html);
    }

    //hide irrelevant months
    let currentYear = new Date().getFullYear();
    let selected_year = parseInt($('.ui-datepicker-year').find('option[selected="selected"]').val());
    if (currentYear === selected_year) {
      let current_month = new Date().getMonth();
      $('.dropdown.months ul li').each(function (idx, el) {
        let selected_month = parseInt($(el).data('value'));
        if (selected_month > current_month) {
          $(el).addClass('hide');
        }
      });
    } else {
      $('.dropdown.months ul li').removeClass('hide');
    }
  }

  function sliceDate(unit) {
    var arr = {};
    var dd = String(unit.getDate()).padStart(2, '0');
    var mm = String(unit.getMonth() + 1).padStart(2, '0'); //January is 0!
    var yyyy = unit.getFullYear();
  ...
  //Date Filtering
  function filterDate(filter_type, This) {
    const is_mobile = window.innerWidth <= 950;
    !is_mobile ? $('#fer-table-content table tbody tr').addClass('hide') : $('.coin-rate-wrapper').addClass('hide');
    let date_to_filter, dateUnit;

    if (This !== false) {
      filter_type = This.data('filter-type');
      $('.filters-container .filter-item').removeClass('marked');
    ...
    switch (filter_type) {
      case 'today':
        let today = new Date();
        dateUnit = sliceDate(today);
        date_to_filter = dateUnit.yyyy + '-' + dateUnit.mm + '-' + dateUnit.dd;
        break;

      case 'yesterday':
        let yesterday = new Date(Date.now() - 864e5);
        dateUnit = sliceDate(yesterday);
        date_to_filter = dateUnit.yyyy + '-' + dateUnit.mm + '-' + dateUnit.dd;
        break;

      case 'dby':
        let dby = new Date(Date.now() - (864e5 * 2));
        dateUnit = sliceDate(dby);
        date_to_filter = dateUnit.yyyy + '-' + dateUnit.mm + '-' + dateUnit.dd;
        break;

      case 'calendar':
        let seletcted_date = $('input#datepicker').val();
        let calendar_unit_arr = seletcted_date.split('/');
        date_to_filter = calendar_unit_arr[2] + '-' + calendar_unit_arr[1] + '-' + calendar_unit_arr[0];
        $('.days-dropdown-select .base-line').addClass('marked');
        break;
    }

    if (!is_mobile) {
      $('#fer-table-content table tbody tr[data-date="' + date_to_filter + '"]').removeClass('hide');
      $('#fer-table-content .last-visible').removeClass('last-visible');
      $('#fer-table-content table tbody tr:not(.hide)').last().addClass('last-visible');
    } else {
      $('.coin-rate-wrapper[data-date="' + date_to_filter + '"]').removeClass('hide');
    }
  }

  function CalendarFooterIndicator() {
    let date = new Date();
    let dateUnit = sliceDate(date);

    let day = parseInt(dateUnit.dd, 10);
    let month = parseInt(dateUnit.mm);
    let year = dateUnit.yyyy;
    var datepicker_month = parseInt($('.ui-datepicker-month').find("option:selected").val()) + 1;
    var datepicker_yearh = parseInt($('.ui-datepicker-year').find("option:selected").val());
    var datepicker_day = parseInt($('.ui-datepicker-current-day').find('a').text());

    if (month === datepicker_month && year === datepicker_yearh && day === datepicker_day) {
      $('.datepicker-footer').addClass('current-day');
    }
  }

  //condense footer accordingly CSS
  function condenceFooter() {
    let lastDate = $('.ui-datepicker-calendar tbody tr').last().index();
    if (lastDate === 5) {
      $('.datepicker-footer').addClass('condense');
    }
  }

  function triggerTodayDate() {
    if ($('input#datepicker').val().length === 0) {
      $('.ui-datepicker-today').click();
    }
  }

  //ES6 PadStart Function support
...
(function ($) {
    ...
    $(document).ready(function () {
      ...
      if ($(window).width() <= 768) {
        $(".sticky-plasma .plasma-btn, .plasma-wrapper .plasma-btn").each(function () {
      ...
      $(".plasma-wrapper").each(function () {
        ...
        if ($(this).attr("data-bool") == "on") {
          setTimeout($(window).scroll(sticky_relocate_left));
    ...
    function sticky_relocate_left() {
        const isBtnExist = $('.plasma-btn-col').find(".plasma-btn").length,
    ...
    function slideStickyPlasma() {
      ...
      if ($(window).width() > 768) {
        changeClassName(".plasma-sticky-desktop", "col-12", "col-8", "col-4");
      } else {
    ...
    function loanMobileWithButtons() {
      ...
      $(".plasma-loan").each(function() {
        ...
        else if($(this).find('.plasma-btn').length) {
...
(function ($) {
  ...
  $(document).ready(function () {
    if (typeof dataLayer == "object") {
      dataLayer.push({
        'event': 'pageview',
        'gtm.allowlist': ['<awct>','<sp>','<gclidw>','<gaawc>','<gaawe>','<ua>','<c>','<e>','<v>','<u>']
      });
    }
  });
		...
		if ( validator.settings.onsubmit ) {
			...
			this.on( "click.validate", ":submit", function( event ) {
				...
				if ( $( this ).attr( "formnovalidate" ) !== undefined ) {
					validator.cancelSubmit = true;
					console.log( $( this ).attr( "formnovalidate" ));
				}
			} );
    ...
    attach: function (context, settings) {
      ...
      $('body').on('mousedown touchstart' ,function (event) {
        ...
        if ($target.hasClass('cookies-close-btn')) {
          ...
          setCookie("banner_cookies",1,ex_days);
          setCookie("banner_cookies_length",cookie_banner_len,ex_days);
          $("#banner-cookies").hide();
          //return false;
        }
      });
...
(function ($) {
    $(document).ready(function () {
        ...
        var targetElem = $('header .header-toolbar ul li.search');
...
(function ($) {
  ...
  function enableCloseCommissionView() {
    $(".taarifon-data span").click(function () {
      $(this).closest(".line-level3-list").removeClass("has-data");
      $(this).closest(".level3-item").slideUp();
      $(this).closest(".level3-item").removeClass("open-operations").find(".level3-title").removeClass("hidden");
    ...
    $(".line-level3-list .level3-title").click(function () {
      ...
      if (!actionTopic.hasClass("open-operations")) {
        ...
        if(actionTopic.parents().hasClass('forex-deposits-container'))
          getForexDepositsCommissionsJson(actionTopic.attr("data-product"), actionTopic.attr("data-option"), actionTopic.attr("data-currency"), lineLevel3List, $(this), $(this).text());
        else
          getTaarifonCommissionsJson(actionTopic.attr("data-item"), actionTopic.attr("data-parent"), $(this).text(), lineLevel3List, actionTopic.attr("data-price-list-num"));
  ...
  function getTaarifonCommissionsJson(id, parentId, title, lineLevel3List, priceListNum) {
    ...
    if ($(".big-business-btn").hasClass('active'))
      ...
      recordType = 24;
    var topic = lineLevel3List.attr("data-topic"), subTopic = lineLevel3List.attr("data-subTopic");
  ...
  $(document).ready(function () {
    ...
    if(flexParagraphChildren && flexParagraphChildren.length > 0) {
      for(var index=0; index < flexParagraphChildren.length; index++) {
        ...
        if ($(elem).hasClass('flex-paragraph')) {
          ...
          while(element.next().hasClass('flex-paragraph')) {
    ...
    function addMainMenuIcon(menuItems, isDesktop) {
      menuItems.each( function () {
        if (isDesktop && window.innerWidth <= 1080) return;
        if (!isDesktop && window.innerWidth > 1080) return;
        ...
        $(tmpImg).one('load', function () {
          ...
          if ($('html').attr("dir") === 'rtl') {
            ...
            if (currentLink.text()) {
              ...
              currentLink.css({"padding":"0", "display": "inline-block", "width" : padding, "min-height": "30px"});
            }
          }
          else {
    ...
    function catchError(event) {
      if (event.data == 100) console.log("Something went wrong..");
    }
    ...
    $(".footer-menu > .menu.links > li > span, .footer-applications-menu  > .menu.links > li > span").attr({"role":"heading", "aria-level": 4});
    ...
    if (is_mobile) {
      ...
      promotionLink.each( function () {
        if ($(this).closest("div").data("url-mobile").length) {
...
M.AutoInit = function (context) {
  ...
  var registry = {
    ...
    Tooltip: root.querySmore-topicselectorAll('.tooltipped:not(.no-autoinit)'),
...
;(function ($, anim) {
  ...
  var Dropdown = function (_Component2) {
    ...
    _createClass(Dropdown, [{
      ...
      value: function _makeDropdownFocusable() {key: "_makeDropdownFocusable",
  ...
  if (M.jQueryLoaded) {
    M.initializeJqueryWrapper(Dropdown, 'dropdownMaterialize', 'M_Dropdown');
  }
})(cash, M.anime);
;(function ($, anim) {
  'use strict';
...
(function ($) {
  ...
  var Parallax = function (_Component5) {
    ...
    function Parallax(el, options) {
      ...
      //if no <img> tag search for 'parallax-element' css class
      if (_this21.$img.length === 0) {
        _this21.$img = _this21.$el.find('.parallax-element').first();
      }
      ...
      this.$img.closest('.parallax-container').addClass("active");
...
;(function ($, anim) {
  ...
  if (M.jQueryLoaded) {
    M.initializeJqueryWrapper(Tooltip, 'tooltipMaterialize', 'M_Tooltip');
  }
})(cash, M.anime);
; /*!
  * Waves v0.6.4
  ...
  var FormSelect = function (_Component20) {
    ...
    _createClass(FormSelect, [{
      ...
      value: function _setupDropdown() {
        ...
        // Add caret
        var dropdownIcon = $('<svg class="caret" height="9" viewBox="0 0 14 9" width="14" xmlns="http://www.w3.org/2000/svg"><path fill="#0A294C" fill-rule="evenodd" d="M7 5.947l-5.293-4.94a1.051 1.051 0 0 0-1.414 0 .89.89 0 0 0 0 1.32l6 5.6c.39.364 1.024.364 1.414 0l6-5.6a.89.89 0 0 0 0-1.32 1.051 1.051 0 0 0-1.414 0L7 5.947z"/></svg>');
